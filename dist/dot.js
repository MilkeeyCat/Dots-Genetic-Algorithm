...
const generateSpotsArr = (start, obstacles, end) => {
    const arr = [];
...
const calcNeighbors = (pos, obstacles, end, spotsArr) => {
    ...
    for (let i = 0; i < obstacles.length; i++) {
        for (let j = 0; j < obstacles.length; j++) {
            let newEnd = end.pos;
            const tempLeft = checkLineIntersection(pos.pos.x, pos.pos.y, newEnd.x, newEnd.y, obstacles[j].pos.x, obstacles[j].pos.y, obstacles[j].pos.x, obstacles[j].pos.y + obstacles[j].height);
            const tempTop = checkLineIntersection(pos.pos.x, pos.pos.y, newEnd.x, newEnd.y, obstacles[j].pos.x, obstacles[j].pos.y, obstacles[j].pos.x + obstacles[j].width, obstacles[j].pos.y);
            const tempRight = checkLineIntersection(pos.pos.x, pos.pos.y, newEnd.x, newEnd.y, obstacles[j].pos.x + obstacles[j].width, obstacles[j].pos.y, obstacles[j].pos.x + obstacles[j].width, obstacles[j].pos.y + obstacles[j].height);
            const tempBottom = checkLineIntersection(pos.pos.x, pos.pos.y, newEnd.x, newEnd.y, obstacles[j].pos.x, obstacles[j].pos.y + obstacles[j].height, obstacles[j].pos.x + obstacles[j].width, obstacles[j].pos.y + obstacles[j].height);
            if (isEnd)
                isEnd = !((tempLeft.onLine2 && tempLeft.onLine1) || (tempTop.onLine1 && tempTop.onLine2) || (tempRight.onLine2 && tempRight.onLine1) || (tempBottom.onLine1 && tempBottom.onLine2));
        }
    }
    if (!isEnd) {
        let newPos = pos.pos;
        for (let i = 0; i < obstacles.length; i++) {
            ...
            for (let j = 0; j < obstacles.length; j++) {
                for (let o = 0; o < arr.length; o++) {
                    const left = checkLineIntersection(newPos.x, newPos.y, arr[o][0], arr[o][1], obstacles[j].pos.x, obstacles[j].pos.y, obstacles[j].pos.x, obstacles[j].pos.y + obstacles[j].height);
                    const top = checkLineIntersection(newPos.x, newPos.y, arr[o][0], arr[o][1], obstacles[j].pos.x, obstacles[j].pos.y, obstacles[j].pos.x + obstacles[j].width, obstacles[j].pos.y);
                    const right = checkLineIntersection(newPos.x, newPos.y, arr[o][0], arr[o][1], obstacles[j].pos.x + obstacles[j].width, obstacles[j].pos.y, obstacles[j].pos.x + obstacles[j].width, obstacles[j].pos.y + obstacles[j].height);
                    const bottom = checkLineIntersection(newPos.x, newPos.y, arr[o][0], arr[o][1], obstacles[j].pos.x, obstacles[j].pos.y + obstacles[j].height, obstacles[j].pos.x + obstacles[j].width, obstacles[j].pos.y + obstacles[j].height);
                    if (res[i][o]) {
